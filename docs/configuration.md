...
既存システムからの移行プロジェクトの場合に設定。分析・設計時に移行元コードの参照指示が含まれる。
...
| キー | 説明 |
|------|------|
| `main_branch` / `develop_branch` | ブランチ運用。`develop_branch` が空なら main に直接マージ |
| `branch_pattern` | 作業ブランチの命名パターン（`{number}` `{summary}` が置換される） |
| `dev_rules` | TEAM.md の開発規約セクションに追記する行（任意・複数行は `\|` ブロック形式推奨） |
